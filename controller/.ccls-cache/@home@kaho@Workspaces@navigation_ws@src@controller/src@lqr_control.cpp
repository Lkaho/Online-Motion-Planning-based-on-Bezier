...
void LQRController::setparams(double sample_t , double L , double thresh , double iter_max)  
{
    _sample_t = sample_t;
    _L = L;
    _thresh = thresh;
...
{
    int min_idx = 0;
    double min_dist  = sqrt(pow((ref[0](0) - current_ps(0)),2) + pow((ref[0](1)- current_ps(1)) , 2));
    for(int i = 1 ; i < ref.size() ; ++i)
    {

        double dist = sqrt(pow((ref[i](0) - current_ps(0)),2) + pow((ref[i](1)- current_ps(1)) , 2));
        if(dist < min_dist)
        {
            min_idx = i;
            min_dist = dist;
...
bool LQRController::compare(Matrix<double , 3 , 3> P_old , Matrix<double , 3 ,3> P_new)
{
    double res;
    for(int i  = 0 ; i < 3; ++i)
        for(int j  = 0 ; j <  3 ;++j)
...
{
    A << 1  , 0  ,  -vel_ref * sin(heading_ref) * _sample_t,
              0 ,  1  ,  vel_ref * cos(heading_ref)  * _sample_t,
              0  ,  0 ,                                    1                                            ;

    B  << cos(heading_ref) * _sample_t ,        0,
                sin(heading_ref) * _sample_t ,        0,
                                            0                               , _sample_t;
}

geometry_msgs::Twist LQRController::LQRsolver(  Vector4d ref_status , Vector4d cur_status)
{
